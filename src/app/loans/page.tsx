    ...
    <div className="min-h-screen bg-zinc-50 pb-20 pt-20 px-4 md:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 className="text-3xl font-bold text-zinc-900 tracking-tight">ระบบยืมของ</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <StatCard label="กำลังยืม" count={loanStats.active} icon="📦" color="bg-blue-50 border-blue-200" />
          <StatCard label="คืนแล้ว" count={loanStats.returned} icon="✓" color="bg-green-50 border-green-200" />
          <StatCard label="เกินกำหนด" count={loanStats.overdue} icon="⚠" color="bg-red-50 border-red-200" />
        </div>
                ...
                >
                  <div className="flex gap-4 items-start flex-1">
                    <div className={`p-3 rounded-lg ${
                      loan.status === 'RETURNED' ? 'bg-green-50 text-green-600' :
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-zinc-900">{loan.itemName}</h3>
                      <p className="text-sm text-zinc-500 mt-1">{loan.description}</p>
                      <div className="flex flex-wrap gap-4 mt-3 text-xs text-zinc-600">
                        ...
                        <span className="flex items-center gap-1">
                          <Calendar size={13} /> {new Date(loan.borrowDate).toLocaleDateString('th-TH')}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 ml-4">
                    <div className="text-right hidden md:block">
                      ...
                      <p className="text-[11px] text-zinc-500 mt-1">ต้องคืน: {new Date(loan.expectedReturnDate).toLocaleDateString('th-TH')}</p>
                    </div>
                    ...
                    <ChevronRight size={20} className="text-zinc-300 group-hover:text-blue-600 group-hover:translate-x-1 transition-all hidden md:block" />
                  </div>
                </div>
              ))}
            </div>
...
function EmptyState() {
  return (
    <div className="p-12 text-center">
      ...
      <h3 className="text-lg font-semibold text-zinc-900 mb-2">ยังไม่มีรายการยืมของ</h3>
